...
class TodoProcessor(Tool):
    ...
    async def execute(self, todo: Todo, user_request: str, file_contexts: List[Dict[str, str]], 
                     completed_todos: List[Todo] = None) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                success=True, 
                data=TodoResult(
                    thinking=f"Error processing todo: {e}", 
                    edits=[], 
                    completed=False, 
                    next_steps="Retry or break down into smaller tasks"
                )
            )
    
    def _build_todo_prompt(self, todo: Todo, user_request: str, file_contexts: List[Dict[str, str]], 
                          completed_todos: List[Todo]) -> str:
